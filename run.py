...
# =================3. 快速训练 & 评估函数=================
def train_and_evaluate(model_name, dataloaders):
    ...
    # --- 简单的训练循环 (Fine-tuning) ---
    print(f"Fine-tuning last layer for {model_name}...")
    model.train()
...
if __name__ == '__main__':
    ...
    for data_path in datasets_list:
        # data_path 已经是相对路径了，例如 'dataset' 或 'dataset_variants/contrast_level_1'
        ...
        print(f"\n\n{'='*20} Processing Dataset: {data_path} {'='*20}")
        
        try:
            dataloaders, _, class_names = get_dataloaders(data_dir)
            print(f"Classes: {class_names}")
            
            for m in models_to_test:
                print(f"\n--- Training {m} on {data_path} ---")
                tn, fp, fn, tp = train_and_evaluate(m, dataloaders)
