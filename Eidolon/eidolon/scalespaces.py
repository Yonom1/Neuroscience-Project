...
#        return np.fft.irfft2((dataPlaneFFT * kernelFFT)) 
    ...
    def Convolve(self, dataPlane, kernel):
        (h,w) = dataPlane.shape
        # Use np.fft.rfft2 for 2D FFT which is more appropriate and robust for images
        # This avoids flattening and potential shape mismatches or ambiguity
        return np.fft.irfft2(np.fft.rfft2(dataPlane) * np.fft.rfft2(kernel), s=(h,w))
    ...
    def Kernel(self, xOrder, yOrder, w, h, sigma):
        ...
        return row * column[:, np.newaxis]    #// Hermite polynomials, via the recursion relation - from utilities
